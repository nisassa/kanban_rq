...
import { TaskType, DragItem, ColumnType } from "../types/kanban.interface";
import { ItemType } from "../types/kanban.enums";
...
export function useDragAndDrop<T extends HTMLElement> (
    ...
    const [{ isDragging }, drag] = useDrag<DragItem, void, { isDragging: boolean }>({
        item: { from: task.column_id, id: task.id, index },
        type: ItemType.TASK,
        collect: (monitor) => ({
...
export function useColumnDrop(
    column: ColumnType,
    handleDrop: (fromColumnID: number, toColumn: number, taskId: TaskType['id']) => void,
) {
    const [{ isOver }, dropRef] = useDrop<DragItem, void, { isOver: boolean }>({
        accept: ItemType.TASK,
        drop: (dragItem) => {
            if (!dragItem || dragItem.from === column.id) {
                return;
            }
            handleDrop(dragItem.from, column.id, dragItem.id);
        },
        collect: (monitor) => ({
